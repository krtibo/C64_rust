...
pub fn operation(opc: &mut Opcode, cpu : &mut MOS6510, operand: u8, logic: Logic) {
    cpu.A = match logic {
        Logic::And => cpu.A & operand,
        Logic::Or => cpu.A | operand,
        Logic::Eor => cpu.A ^ operand,
    };
...
pub fn and_29(opc: &mut Opcode, cpu : &mut MOS6510) {
    ...
    operation(opc, cpu, operand, Logic::And);
    cpu.cycle += 2;
}
...
pub fn ora_09(opc: &mut Opcode, cpu : &mut MOS6510) {
    ...
    operation(opc, cpu, operand, Logic::Or);
    cpu.cycle += 2;
}
...
pub fn eor_49(opc: &mut Opcode, cpu : &mut MOS6510) {
    ...
    operation(opc, cpu, operand, Logic::Eor);
    cpu.cycle += 2;
}
